...
## LLM 관측성 — Langfuse를 붙이면 무엇이 바뀌는가 (2026-09-24)

**목표·이유.** 이 코파일럿은 이미 자체 JSONL 트레이서(`app/trace.py`)와 운영 대시보드를 갖고 있다.
업계 표준 LLM 관측 도구인 Langfuse를 셀프호스팅(`langfuse-selfhost/`, v4, Docker — 비용 0)으로
**대체가 아니라 병행**으로 붙이고(`app/langfuse_trace.py`, 키가 없으면 완전한 no-op), "붙였다"가
아니라 **붙이면 요청 지연·처리량·장애 내성·데이터 유실이 어떻게 바뀌는지**를 쟀다.

**측정 조건.** 결정적 평가셋(golden+adversarial 28건, SQL·비전은 스텁이라 LLM 키·네트워크 잡음 없음)을
모드별로 5라운드(140요청) 반복. Langfuse SDK v4는 OpenTelemetry 전역 상태를 잡으므로 **모드마다
별도 프로세스**로 돌렸다. 도착 여부는 v2 observations API로 span 수를 세어 자체 트레이서 기준 기대값과 대조.

```bash
cd langfuse-selfhost && docker compose up -d      # http://localhost:3000
python tools/bench_langfuse.py --rounds 5         # 지연·정확성·유실·서버 다운
python tools/bench_langfuse_crash.py              # 강제 종료 시 유실(배치의 대가)
```

| 모드 | 요청 지연 중앙값 | p95 | 오버헤드 | e2e 결과 | span 도착 |
|---|---|---|---|---|---|
| 계측 끔(기준선) | 0.59ms | 0.88ms | — | 0.857 | — |
| 요청마다 flush (최초 구현) | 57.25ms | 64.81ms | **+56.66ms** | 동일 | 325/325 |
| **배치 전송 (교체 후 기본값)** | 0.94ms | 1.73ms | **+0.35ms** | 동일 | 325/325 |
| 서버 다운 + 요청마다 flush | **4,071ms** | 5,138ms | +4.07s | 동일(요청 실패 0) | — |
| 서버 다운 + 배치 | 1.07ms | 1.80ms | +0.48ms | 동일(요청 실패 0) | — |

- **계측은 동작을 바꾸지 않았다**: 다섯 모드 모두 케이스별 e2e 결과가 기준선과 완전히 같다.
- **유실 0**: 정상 서버에서는 두 전송 방식 모두 기대 span 325개가 전부 도착했다(루트 140 + 에이전트 단계 185).
- **요청마다 flush가 치명적이었다**: 요청 경로에서 동기 전송을 기다려 지연이 약 100배가 됐고,
  Langfuse 서버가 죽으면 **요청 하나당 4초씩** 멈췄다. 관측 도구의 장애가 서비스 장애로 번지는 구조다.

**교체 기록 — 배치로 바꾸고, 그 대가를 쟀다.** 기본값을 `LANGFUSE_FLUSH_EACH=0`(SDK 백그라운드
배치, 정상 종료 시 atexit flush)으로 바꿨다. 배치의 대가는 "아직 안 보낸 묶음"이라, 크래시를
`os._exit()`(atexit 건너뜀)로 재현해 12초 부하(요청 간 50ms) 직후 강제 종료했다. 3회 반복:

| 전송 방식 | 12초간 처리한 요청 | 강제 종료 시 span 유실 |
|---|---|---|
| 요청마다 flush | 103~106건 | 0% (3회 모두) |
| 배치, 전송 주기 5초(SDK 기본) | 219~222건 | 11.6~12.2% |
| 배치, 전송 주기 1초 | 221~222건 | 7.6~8.4% |

- 유실량은 **마지막 전송 이후 쌓인 트래픽**만큼이다 — 1초 주기에서 잃은 약 40개는 이 부하의 1초치와 같다.
  주기를 줄여도 0이 되지는 않는다.
- 같은 12초에 요청마다 flush는 요청을 **절반밖에 처리하지 못했다**(처리량 약 2.1배 차이).
- 결론: 대화형 코파일럿에서 요청당 +57ms·서버 다운 시 4초 정지는 "크래시 순간의 트레이스 약 1초치"보다
  비싸다. 그 1초치는 **병행 중인 자체 JSONL 트레이서가 요청마다 파일에 즉시 기록**하므로 남는다 —
  Langfuse를 대체가 아니라 병행으로 붙인 설계가 여기서 값을 한다. 권장 설정은 `LANGFUSE_FLUSH_INTERVAL=1`.

**에러 대처 기록.**
- 도착 확인에 처음 쓴 조회 경로(`/api/public/traces`, `/api/public/observations`)가 404였다. v4 셀프호스팅은
  events-only 모드라 구 엔드포인트를 닫고 `/api/public/v2/observations`만 연다 — 도착 확인을 v2로 옮겼다.
- 한 프로세스 안에서 모드를 바꾸면 SDK가 잡은 OTel 전역 TracerProvider가 남아 호스트·키 변경이 안 먹는다 →
  모드별 서브프로세스로 격리.
- 벤치 중 `pytest`를 돌리면 테스트가 `.env`의 실제 키로 트레이스를 보내 도착 수가 오염된다 → 테스트는 키를
  비운 채 실행하고, 계측 모듈 테스트(`tests/test_langfuse_trace.py`)는 가짜 클라이언트로 돈다.

**정직한 범위.** 스텁 경로라 LLM generation span(`llm.complete`)은 이 실측에 포함되지 않았다(키·쿼터 없이
재현하기 위해). 셀프호스팅 서버와 앱이 같은 PC라 네트워크 지연은 LAN보다도 짧은 조건이다 — 원격 서버라면
